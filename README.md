...
- `Слушатели событий` : Расположены в основном коде проекта и реагируют на инициированные моделями события. Они выполняют необходимые вычисления или обработку данных и передают их компонентам отображения для последующего визуального представления.
...
- Функция-страж `isModel` : Функция для проверки, является ли объект экземпляром `Model`.
##### Конструктор
`constructor(data: Partial<T>, events: IEvents)` - Инициализирует модель данными и подключает систему событий.
##### Методы
...
Класс `Model` является центральным звеном в архитектуре проекта, обеспечивая хранение и управление данными. С помощью метода `emitChanges` может активно уведомлять другие компоненты проекта об изменениях.
...
#### Структура и назначение
Класс `AppState` является центральной моделью для управления состоянием проекта, включая каталог продуктов, корзину, информацию о заказе и ошибки валидации форм. Этот класс расширяет базовый класс `Model`, что позволяет использовать возможности по управлению данными и событиями.   
Свойства :   
...
- `addToBasket(item: Product)` : Добавляет товар в корзину, если он там еще не присутствует.
...
- `setPreview(item: Product)` : Устанавливает товар для предпросмотра.
...
#### Типы данных
- `IAppState` : Интерфейс для общего состояния проекта. Включает в себя поля для каталога продуктов, корзины, текущего заказа, предпросмотра продукта и ошибок форм.
...
- `options(RequestInit)` : Настройки для запросов, включая заголовки.
##### Конструктор
`constructor(baseUrl: string, options: RequestInit)` - Инициализирует класс с указанным базовым URL и настройками запросов.
##### Методы
...
- `post<T>(uri: string, data: object, method: ApiPostMethods): ApiResponse<T>` : Выполняет POST-запрос с данными `data` по указанному URI. Возвращает промис с типизированным ответом от сервера.
#### Типы данных
...
##### Методы
- `initEventListeners(): void` : Метод для инициализации обработчиков событий. Устанавливает обработчики для кнопки закрытия и внешнего контейнера модального окна.
...
`Modal` используется для отображения важной информации, форм или подтверждений во всплывающем окне,не переходя на другую страницу. Он взаимодействует с пользователем и может генерировать события, которые обрабатываются в других частях приложения.
...
##### Методы
- `handleInput(e: Event): void` : Обрабатывает ввод в поля формы, вызывая 'onInputChange' для каждого измененного поля.
- `handleSubmit(e: Event): void` : Обрабатывает событие отправки формы, предотвращая стандартное поведение и инициируя событие отправки.
- `onInputChange(field: keyof T, value: string): void` : Вызывается при изменении значений полей формы. Эмитирует событие изменения с новыми значениями.
...
- `render(state: Partial<T> & IFormState): HTMLElement` : Рендерит форму с новым состоянием, обновляя валидность и ошибки.
#### Типы данных
...
##### Методы
- `toggleCheckoutButton(isDisabled: boolean): void` : Включает или выключает кнопку оформления заказа в зависимости от наличия товаров в корзине.
...
##### Методы
- `initializePaymentButtons(actions?: IActions)` : Инициализирует кнопки выбора способа оплаты и назначает им обработчики событий.
- `toggleStateButtons(target: HTMLElement)` : Переключает активное состояние кнопок способа оплаты.
...
#### Взаимодействие с другими частями приложения и использование в архитектуре
Этот класс используется для создания интерактивной формы заказа, где пользователи могут выбрать способ оплаты и указать адрес доставки. Класс обеспечивает интеграцию с системой событий для обработки пользовательского ввода и действий.
...
- `events: IEvents` : Устанавливает значение в поле электронной почты.
#### Типы данных
- `IContactForm` : Интерфейс, описывающий структуру данных для формы заказа.
#### Взаимодействие с другими частями приложения и использование в архитектуре
Класс используется для создания формы, через которую пользователи могут ввести свои контактные данные, такие как телефон и электронная почта. Поддерживает интеграцию с системой событий для обработки ввода данных и их валидации.
...
Свойства :   
- `_closeButton: HTMLElement` : Кнопка для закрытия сообщения об успехе.
...
##### Методы
- `set total(value: string)` : Устанавливает текст, отображаемый в элементе `_totalElement`. Используется для обновления информации о стоимости или других данных, связанных с успешным завершением операции.
#### Типы данных
